...
describe('Coupled Emergence Layer v1', () => {
  test('activity produces correct perturbation proposals', () => {
    const proposals = coupleActivityToFields([
      ...
      { action: 'cast_spark', tileId: 'town' }
    ]);

    expect(proposals[0]).toMatchObject({
      tileId: 'town',
      source: 'activity',
      fields: { fire: 0.12, earth: 0.08, arcane: 0.12 }
    });
  ...
  test('coupling runs after field tick and queues proposals for next tick', () => {
    ...
    expect(world.lastFieldDynamicsTrace.preDiffusionState.town.arcane).toBeLessThan(0.12);
    expect(world.fieldPerturbationQueue[0].perturbation.arcane).toBe(0.12);

    tickManager([], world);
    expect(world.lastFieldDynamicsTrace.preDiffusionState.town.arcane).toBeGreaterThanOrEqual(0.12);
  });
