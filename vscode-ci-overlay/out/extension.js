...
function activate(context) {
    const cmd = vscode.commands.registerCommand('ciGraph.open', () => {
        ...
        panel = vscode.window.createWebviewPanel('ciGraph', 'CI Graph Overlay', vscode.ViewColumn.Two, { enableScripts: true, retainContextWhenHidden: true });
        const filePath = path.join(vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? '', 'docs', 'ci', 'graph', 'latest.json');
        const sendUpdate = () => {
